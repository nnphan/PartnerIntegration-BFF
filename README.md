...
Built as a Clean Architecture solution with full unit/integration test coverage,
structured logging, global exception handling, API key auth, and Docker support.
...
- **FluentValidation for request validation**, run explicitly inside
  `TransactionService` rather than via an MVC filter, so the same validation
  path is exercised identically whether the request arrives over HTTP or
  (hypothetically) from another entry point later.
- **Polly policies isolated in `PartnerVerificationPolicies`**, registered via
...
- **RabbitMQ publisher declares the queue as durable and publishes persistent
  messages**, trading a small throughput cost for messages surviving a broker
  restart, which matches the transactional nature of partner payments.
